 ...
 */
 ...
 module.exports = function (input, output) {
     ...
     if (output.width && output.height) {
         var fn = (output.mode === 'pad' || output.mode === 'meet') ? Math.min : Math.max;
     ...
     resize.left = ((input.left || 0) * resize.scale) + (((input.width * resize.scale) - resize.width) / 2);
     resize.top = ((input.top || 0) * resize.scale) + (((input.height * resize.scale) - resize.height) / 2);
